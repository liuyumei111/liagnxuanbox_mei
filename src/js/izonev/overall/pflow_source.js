'use strict';
app.controller('osourceCtrl', ['$scope', '$http', '$state', '$q', '$interval', function ($scope, $http, $state, $q, $interval) {
    ...
    if (window.sessionStorage.getItem("ProjectName") == "changscz" || window.sessionStorage.getItem("ProjectName") == "xiyuan") {
        $scope.pars = "null";
    } else {
        $scope.pars = "ALL";
    }
    ...
    var mainHttp = {
        ...
        districtRanking: function (chart_id, pid, aid, type, province, city, district, start_date, end_date) {

            var deferred = $q.defer();
            $http({
                method: 'get',
                url: $scope.app.apiUrl + "getPortrayalDatas?" + "chart_id=" + chart_id + "&pid=" + pid + "&aid=" + aid + "&type=" + type + "&province=" + province + "&city=" + city + "&district=" + district + "&start_date=" + start_date + "&end_date=" + end_date,
                headers: {'Authorization': "Bearer " + $scope.app.token}, withCredentials: true
            }).success(function (res) {
                if (res.code === 2000) {
    ...
    //"20171103","20171101","home","izone_"+window.sessionStorage.getItem("pid")+"_src",window.sessionStorage.getItem("key")+"_Area_Top"
    var initChart = {
        ...
        runmap: function (lng, lat, zoom, data, typeMap) {
            ...
            $("#province10").change(function () {
                ...
                mainHttp.districtRanking(audienceSource.community, $scope.pid, "ALL", $scope.selectHomeOrWork, province, "ALL", "ALL", $scope.app.exhibition.startDate, $scope.app.exhibition.endDate);
                $scope.setmpaType();
            });
            $("#city10").change(function () {
                ...
                mainHttp.districtRanking(audienceSource.community, $scope.pid, "ALL", $scope.selectHomeOrWork, province, city, "ALL", $scope.app.exhibition.startDate, $scope.app.exhibition.endDate);
                $scope.setmpaType();
            });
            $("#district10").change(function () {
                ...
                mainHttp.districtRanking(audienceSource.community, $scope.pid, "ALL", $scope.selectHomeOrWork, province, city, district, $scope.app.exhibition.startDate, $scope.app.exhibition.endDate);
                $scope.setmpaType();
            })
    ...
    var init = function (url) {
        ...
        mainHttp.getSourceContrastProv(audienceSource.Prov_id, $scope.pid, "ALL", $scope.selectHomeOrWork, startDate, endDate).then(function () {
            mainHttp.getSourceContrastCity(audienceSource.City_id, $scope.pid, "ALL", $scope.selectHomeOrWork, startDate, endDate).then(function () {
                mainHttp.getSourceContrastForeign(audienceSource.overseas_id, $scope.pid, "ALL", $scope.selectHomeOrWork, startDate, endDate).then(function () {
                    initChart.customerFrom($scope.pie.dist, $scope.pie.city, $scope.pie.prov, $scope.pie.foreign, "customerFromContrastID");
                });
            });
        });
        mainHttp.getHeatMap(audienceSource.chart_id, $scope.pid, "ALL", $scope.selectHomeOrWork, startDate, endDate).then(function () {

            initChart.runmap(lng, lat, zoom, $scope.HeatMapRes, "mapHot");
            $scope.loading = false;
        });
        mainHttp.districtRanking(audienceSource.community, $scope.pid, "ALL", $scope.selectHomeOrWork, "ALL", "ALL", "ALL", startDate, endDate);
